...
Repo scaffolding done (LICENSE files, DECISIONS.md, CONTRIBUTING.md, README, Dockerfile). Slice 1 done: `scrapers/amnesty.py` ingests Amnesty's RSS feed (`https://www.amnesty.org/en/feed/`) rather than scraping HTML — see the 2026-07-16 "Amnesty ingestion via RSS feed" entry in `DECISIONS.md` for why, and its "country vs. region split" caveat (a maintained `REGION_NAMES` list, not derived from the feed). Ingestion is narrowed to resource type `Action`/`Urgent Action` via `filter_by_resource_type()` (see the "Narrow ingestion to Action / Urgent Action resource types" DECISIONS.md entry) — **but this filter currently yields close to zero events**, because that content lives almost entirely at `amnesty.org/en/documents/...` URLs, which the RSS feed, the public REST API, and the XML sitemap all fail to expose (verified live on 2026-07-16). A different ingestion path for `/en/documents/` content is needed before this filter does real work — not yet built. `scrapers/report.py` generates a run report (events found, date range, per-country counts, skipped items) after every run; reuse it for future adapters. Tested against `tests/fixtures/amnesty/feed_sample.xml`; passes via `docker run --rm hr-response-tracker pytest` (17 tests). Does not yet determine the perpetrating actor or filter to state-perpetrated violations (that LLM classification call is still to come). Next: resolve the `/en/documents/` ingestion gap, then the ministry-side adapter interface starting with US State Dept (build order item 2).
