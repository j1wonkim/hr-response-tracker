...
A fully automated, public-facing pipeline that tracks how major states respond (or stay silent) when human rights organizations — Amnesty International and Human Rights Watch, with room for more — document state-perpetrated human rights violations. Data lives in this public GitHub repo, updates daily via GitHub Actions, and is visualized on GitHub Pages (later integrated into my personal website).
...
1. **Event ingestion (human rights org side).** Scrape human rights organizations' event-level content daily, RSS where available: Amnesty International (Urgent Actions and news, /latest/news/) and Human Rights Watch (/news, resource types News Release/Statement). Extract: date, target country, perpetrating actor, summary text, URL. Filter to state-perpetrated violations only (an LLM classification call: "Is the government the responsible actor?"). The state is treated as the duty bearer. Each source is narrowed to resource/news types that represent a discrete, datable event — not ongoing multi-year documentation (reports, annual reviews, research briefings) — since those don't fit a real-time response tracker; see DECISIONS.md. Once sources are combined, deduplicate events reported by more than one organization (shared country + similar title + close publication date) before they reach linking, so one real-world incident doesn't produce two rows in the response matrix; see DECISIONS.md.
...
Repo scaffolding done (LICENSE files, DECISIONS.md, CONTRIBUTING.md, README, Dockerfile). Slice 1 (Amnesty) done: `scrapers/amnesty.py` ingests Amnesty's RSS feed rather than scraping HTML — see "Amnesty ingestion via RSS feed" in `DECISIONS.md`. Narrowed to resource type `Action`/`Urgent Action`, but **this filter currently yields close to zero events**: that content lives almost entirely at `amnesty.org/en/documents/...` URLs, which the RSS feed, REST API, and sitemap all fail to expose (verified live 2026-07-16; not yet resolved). Added Human Rights Watch as a second source: `scrapers/hrw.py` fetches `hrw.org/rss/news` for the item index, then fetches+parses each article page for country/topic/news-type tags (that taxonomy isn't in HRW's feed, unlike Amnesty's) — narrowed to news type `News Release`/`Statement`. Unlike Amnesty, this one actually works: a live run on 2026-07-17 produced 9 real events from 20 raw feed items. See "Add Human Rights Watch as a second event source" in `DECISIONS.md`, including a known nuance where a US policy-area label was seen tagged as if it were a country. `scrapers/http.py` holds the shared polite-fetch-with-cache helper (now used by both sources); `scrapers/report.py` generates a run report (events found, date range, per-country counts, skipped items, duplicates merged) after every run — reuse both for future adapters. `scrapers/dedup.py` merges same-incident events reported by more than one source (heuristic: shared country + title similarity + 3-day window, not yet LLM-verified — see DECISIONS.md); `scrapers/pipeline.py` runs both sources, dedups, and writes one combined `data/events.json` + run report. 33 tests pass via `docker run --rm hr-response-tracker pytest`. Neither source determines the perpetrating actor or filters to state-perpetrated violations yet (that LLM classification call is still to come). Next: resolve the Amnesty `/en/documents/` ingestion gap (optional/lower priority now that HRW covers discrete events), then the ministry-side adapter interface starting with US State Dept (build order item 2).
